...
#                c=np.arange(centroids_3d.shape[0]).astype(np.float), edgecolor='k', marker='X', s=100)
    ...
    ax1.set_title('unclustered data points')
...
#     ax1.set_title('Iteratoin: %i' %(i+1))
    ...
    ax2.set_title('clustering iteratoin: %i' %(i+1))
...
anim.save('Kmeans_iterations_raw+km.mp4', fps=1, writer='ffmpeg')
